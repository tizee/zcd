...
impl<'a> DbServer<'a> {
    ...
    fn listen(self) -> Result<()> {
        ...
        loop {
            ...
            thread::scope(|s| {
                s.spawn(|_| that.handle_connection(stream, debug));
            })
            .unwrap();
        }
    }
    ...
    fn handle_connection(&self, stream: UnixStream, debug: bool) -> Result<()> {
        ...
        match msg {
            Some(ServerOps::Insert(path)) => {
                println!("insert {}", path);
                self.db.write().unwrap().insert_or_update(path.into());
                Ok(())
            }
            Some(ServerOps::Delete(path)) => {
                println!("delete {}", path);
                self.db.write().unwrap().delete(path.as_str());
                Ok(())
            }
            Some(ServerOps::Query(pattern)) => {
                println!("query {}", pattern);
                if let Some(dir) = self.db.read().unwrap().query(pattern.as_str()) {
            ...
            Some(ServerOps::List) => {
                println!("get list");
                if let Some(list) = self.db.read().unwrap().list() {
                    let mut writer = BufWriter::new(stream);
...
fn get_message<T: DeserializeOwned>(
    ...
    serde_json::from_slice(buf).context("failed to parse")
}
